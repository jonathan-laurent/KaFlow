...
let find_eois ~env ~rule id step eois =
  match step with
  | Trace.Subs _ | Trace.Dummy _ | Trace.Init _ | Trace.Pert _ -> eois
  | Trace.Rule (rid, _, _) ->
      if Some (Causal_core_util.rule_ast_name env rid) = rule
      then id :: eois else eois
  | Trace.Obs _ ->
    if rule = None then id ::eois else eois
...
let main () =
  ...
  if !file = "" then
    ...
    let eois = te |> Trace_explorer.fold ~init:[] (find_eois ~env ~rule) in
